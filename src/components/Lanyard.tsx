...
import cardGLB from '../resources/3d/card.glb?url'
import lanyard from '../resources/img/lanyard/lanyard.png'
...
type MeshLineGeo = InstanceType<typeof MeshLineGeometry>
type MeshLineMat = InstanceType<typeof MeshLineMaterial>
...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: BandProps) {
  const band = useRef<THREE.Mesh<MeshLineGeo, MeshLineMat>>(null)
  ...
  const lerpMap = useRef(new Map<RapierRigidBody, THREE.Vector3>())
  ...
  const nodes = gltf.nodes as CardGLTF['nodes']
  const materials = gltf.materials as CardGLTF['materials']
  ...
  useFrame((state, delta) => {
    ...
    ;[j1, j2].forEach((ref) => {
      ...
      const stored = lerpMap.current.get(ref.current) ?? new THREE.Vector3().copy(ref.current.translation())
      const clampedDistance = Math.max(0.1, Math.min(1, stored.distanceTo(ref.current.translation())))
      stored.lerp(ref.current.translation(), delta * (minSpeed + clampedDistance * (maxSpeed - minSpeed)))
      lerpMap.current.set(ref.current, stored)
    })

    curve.points[0].copy(j3.current.translation())
    curve.points[1].copy(lerpMap.current.get(j2.current) ?? j2.current.translation())
    curve.points[2].copy(lerpMap.current.get(j1.current) ?? j1.current.translation())
          ...
          >
            <mesh geometry={(nodes.card as THREE.Mesh).geometry}>
              <meshPhysicalMaterial
                map={materials.base.map as THREE.Texture}
